...
2. `codegen_stage`：按候选生成代码。
3. `optimize_stage`：调用优化器，原地修改算法文件。
4. `execute_stage`：执行算法脚本。
5. `evaluate_stage`：评估质量与延迟。
7. `package_stage`：写入 `report.json` 与阶段记录。

禁止跳过 `package_stage` 与日志落盘。
## 3) Retry 与 Fallback 策略
...
2. 失败路径可在 `events.jsonl` 与 `report.json` 中定位。
3. 未引入破坏契约的签名变更。
4. 所有新增行为可在现有目录结构中追溯。
