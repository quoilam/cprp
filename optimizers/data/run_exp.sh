...
claude -p "Read program.md carefully. It contains AUTO_MODE instructions. You must NOT ask for confirmation. Execute the full experiment setup and loop immediately. Start now." --dangerously-skip-permissions --output-format stream-json --verbose | python3 - <<'PY'
import json
import sys
    ...
    try:
        ...
        if t == "assistant":
            ...
            if content.get("type") == "text":
                ...
                if name == "Bash":
                    print(f"\n \033[33mExecuting command:\033[0m\n   \033[90m{tool_input.get('command')}\033[0m")
                elif name == "Read":
                    print(f"\n \033[33mReading file:\033[0m\n   \033[90m{tool_input.get('file_path')}\033[0m")
